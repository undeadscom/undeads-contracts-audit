# IMarketplaceTreasury.sol

View Source: [\contracts\marketplace\interfaces\IMarketplaceTreasury.sol](..\contracts\marketplace\interfaces\IMarketplaceTreasury.sol)

**↗ Extends: [IClaimableFunds](IClaimableFunds.md)**
**↘ Derived Contracts: [MarketplaceTreasury](MarketplaceTreasury.md)**
...
```solidity
function addFunds(address paymentAsset_, address beneficiar_, uint256 amount_) external nonpayable
```
...
```solidity
function approvePayment(address paymantAddress_, bool enabled_) external nonpayable
```
